...
function appearanceTabMarkup() {
  ...
  const companions=COMPANION_DEFS.map(x=>{const unlocked=companionUnlocked(x);const lockedAttr=unlocked?"":"data-locked=\"1\"";return `<button class="companion-tile ${companionId===x.id?"selected":""} ${unlocked?"":"locked"}" data-companion="${x.id}" ${lockedAttr}><img src="${companionAsset(x)}" alt=""><span><b>${x.name}</b><small>${unlocked?x.role:companionUnlockLabel(x)}</small></span><em>${unlocked?(companionId===x.id?"Напарник":"Выбрать"):(x.rewardText||"Открывается позже")}</em></button>`;}).join("");
