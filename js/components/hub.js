...
function themeUnlocked(def) {
  return Math.max(profile.totalStars || 0, profile.cosmeticStarsPeak || 0) >= def.stars;
}
...
function appearanceTabMarkup() {
  ...
  const icons=APP_ICON_DEFS.map(x=>`<button class="app-icon-tile ${profile.settings.appIcon===x.id?"selected":""}" data-app-icon="${x.id}"><img src="${x.apple}" alt=""><b>${x.name}</b><span>${profile.settings.appIcon===x.id?"Выбрано":"Выбрать"}</span></button>`).join("");
...
function bindHubHandlers() {
  ...
  hubContent.querySelectorAll("[data-app-icon]").forEach((btn)=>btn.onclick=()=>{profile.settings.appIcon=appIconDef(btn.dataset.appIcon).id;applyAppIcon(profile.settings.appIcon);saveProfile();showToast("Иконка выбрана. Если ярлык уже стоит на iPhone, добавь его на экран «Домой» заново.");renderHub();});
