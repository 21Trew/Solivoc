...
function appearanceTabMarkup() {
  ...
  const companionId=companionDef(profile.settings.companion).id;
  const companions=COMPANION_DEFS.map(x=>`<button class="companion-tile ${companionId===x.id?"selected":""}" data-companion="${x.id}"><img src="${x.image}" alt=""><span><b>${x.name}</b><small>${x.role}</small></span><em>${companionId===x.id?"Напарник":"Выбрать"}</em></button>`).join("");
  ...
  const companionShelf=`<section class="hub-section companion-section"><div class="hub-section-head"><div><h3>Маскот-напарник</h3><small>помогает с подсказками и встречает после уровня</small></div></div><div class="companion-grid">${companions}</div></section>`;
...
function bindHubHandlers() {
  ...
  hubContent.querySelectorAll("[data-companion]").forEach((btn)=>btn.onclick=()=>{profile.settings.companion=companionDef(btn.dataset.companion).id;saveProfile();syncGameCompanion?.();showToast(`${companionDef(profile.settings.companion).name} теперь твой напарник`);renderHub();});
