...
function chapterFinalSpecial(level) {
  ...
  if (everyThirdFinal) {
    const bosses = [
      ...
      { id: "bear", icon: "🐻", name: "Сильный медведь", taunts: ["Здесь побеждает не спешка, а выдержка.", "Давить на тебя я не буду — расклад сам всё проверит.", "Покажи, что умеешь держать удар и думать спокойно."] },
      { id: "raven", icon: "🐦‍⬛", name: "Умный ворон", taunts: ["Я замечаю ошибки раньше, чем ты тянешься к карте.", "Ну что, блеснёшь умом — или только перья распушишь?", "Люблю смотреть, как самоуверенность спотыкается о детали."] },
      { id: "wolf", icon: "🐺", name: "Серый волк", taunts: ["След верный только один — попробуй его не потерять.", "Тут побеждает тот, кто идёт до конца без лишнего шума.", "Я уже взял след. А ты успеешь за раскладом?"] },
      { id: "tiger", icon: "🐯", name: "Грозный тигр", taunts: ["Сейчас начнётся уровень, который любит сильных.", "Одно неверное движение — и я уже слышу треск твоего плана.", "Пора показать, чего стоят твои нервы."] },
      { id: "panda", icon: "🐼", name: "Спокойная панда", taunts: ["Без суеты. Только ясная голова и точные решения.", "Тише едешь — дальше проходишь. Проверим?", "Если начнёшь торопиться, расклад сразу это почувствует."] },
