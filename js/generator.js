...
function chapterFinalSpecial(level) {
  ...
  if (everyThirdFinal) {
    const bosses = [
      ...
      { id: "wolf", icon: "🐺", name: "Волк-следопыт", taunts: ["Твой след обрывается здесь.", "Назад дороги уже нет.", "Посмотрим, кто лучше чувствует расклад."] },
      { id: "tiger", icon: "🐯", name: "Тигр-задавака", taunts: ["Слишком тихо. Сейчас будет сложнее.", "Мои полосы уже считают твои ошибки.", "Попробуй удивить меня."] },
      { id: "panda", icon: "🐼", name: "Панда-стратег", taunts: ["Я никуда не спешу. А ты?", "Суета здесь только помешает.", "Докажи, что умеешь думать спокойно."] },
      { id: "frog", icon: "🐸", name: "Лягуш-алхимик", taunts: ["Рецепт твоего поражения уже готов.", "Ква-ква. Это будет интересно.", "Смешаем немного хаоса?"] },
      { id: "octopus", icon: "🐙", name: "Осьминог-профессор", taunts: ["У меня восемь рук. А сколько планов у тебя?", "Я уже просчитал несколько твоих ходов.", "Экзамен начинается прямо сейчас."] },
    ];
    const bossIndex = Math.max(0, Math.floor(chapter / 3) - 1);
