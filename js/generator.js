...
function chapterFinalSpecial(level) {
  ...
  if (everyThirdFinal) {
    const bosses = [
      { id: "fox", icon: "🦊", name: "Хитрый лис", taunts: ["А силёнок хватит?", "Лучше сдайся сразу.", "Я уже спрятал победу в рукаве."] },
      { id: "bear", icon: "🐻", name: "Сильный медведь", taunts: ["Эту главу я никому не отдаю.", "Посмотрим, хватит ли тебе терпения.", "Можешь разворачиваться, если не уверен."] },
      { id: "raven", icon: "🐦‍⬛", name: "Умный ворон", taunts: ["Слишком самоуверенно для новичка.", "Ты опоздал к своей победе.", "Посмотрим, как ты выкрутишься теперь."] },
      { id: "wolf", icon: "🐺", name: "Серый волк", taunts: ["Твой след обрывается здесь.", "Назад дороги уже нет.", "Посмотрим, кто лучше чувствует расклад."] },
      { id: "tiger", icon: "🐯", name: "Грозный тигр", taunts: ["Сейчас станет по-настоящему сложно.", "Мои полосы уже считают твои ошибки.", "Попробуй удивить меня."] },
      { id: "panda", icon: "🐼", name: "Спокойная панда", taunts: ["Я никуда не спешу. А ты?", "Суета здесь только помешает.", "Докажи, что умеешь думать спокойно."] },
      { id: "frog", icon: "🐸", name: "Ловкая лягушка", taunts: ["Ква-ква. Это будет интересно.", "Смешаем немного хаоса?", "Рецепт твоего поражения уже почти готов."] },
      { id: "octopus", icon: "🐙", name: "Умный осьминог", taunts: ["У меня восемь рук. А сколько планов у тебя?", "Я уже просчитал несколько твоих ходов.", "Экзамен начинается прямо сейчас."] },
    ];
