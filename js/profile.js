...
function defaultProfile() {
  return {
    ...
    collectibles: { version: 1, unlocked: [], discovered: [], seen: [] },
    visualDiscovered: [],
    levelRecords: {},
    dailyRecords: {},
    challengeRecords: {},
  ...
  for (const key of keys) {
    try {
      ...
      return {
        ...
        birthdayWeek: { ...defaultProfile().birthdayWeek, ...(p.birthdayWeek || {}) },
        ...
        collectibles: {
          version: 1,
          unlocked: Array.isArray(p.collectibles?.unlocked) ? [...new Set(p.collectibles.unlocked.map(String))].slice(0, 500) : [],
          discovered: Array.isArray(p.collectibles?.discovered) ? [...new Set(p.collectibles.discovered.map(String))].slice(0, 500) : [],
          seen: Array.isArray(p.collectibles?.seen) ? [...new Set(p.collectibles.seen.map(String))].slice(0, 500) : [],
        },
