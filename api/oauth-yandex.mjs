...
function callbackUri(request) {
  const configured = String(process.env.YANDEX_REDIRECT_URI || "").trim();
  if (configured) return configured;
  const url = new URL(request.url);
  return `${url.origin}/api/oauth-yandex`;
}
...
async function exchangeCode(request, code) {
  const clientId = String(process.env.YANDEX_CLIENT_ID || "").trim();
  const clientSecret = String(process.env.YANDEX_CLIENT_SECRET || "").trim();
...
async function fetchYandexIdentity(accessToken) {
  ...
  const data = await response.json().catch(() => ({}));
  const providerId = String(data?.id || "").replace(/[^a-zA-Z0-9_-]/g, "").slice(0, 128);
  const email = cleanEmail(data?.default_email || data?.emails?.[0] || "");
...
export async function GET(request) {
  ...
  try {
    if (!(await checkRateLimit(request, "oauth-yandex", 30, 900))) return redirect(redirectResult(returnTo, "error", "rate_limited"));
    const url = new URL(request.url);
    ...
    const identity = await fetchYandexIdentity(accessToken);
    ...
    console.error("oauth yandex", error?.code || error?.message || error);
