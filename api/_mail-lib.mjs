...
function authEmailContent(kind, code) {
  if (kind === "recovery") {
    return {
      subject: "Код восстановления доступа — Словасьянс",
      heading: "Сброс пароля",
      intro: "Введи этот код в игре, чтобы задать новый пароль. Он действует 10 минут.",
      textIntro: "Используй этот код, чтобы сбросить пароль в Словасьянсе.",
      footer: "Если ты не запрашивал восстановление доступа, просто проигнорируй письмо. Пароль не изменится.",
    };
  }
  return {
    subject: "Код подтверждения — Словасьянс",
    heading: "Подтверди регистрацию",
    intro: "Введи этот код в игре. Он действует 10 минут.",
    textIntro: "Подтверди регистрацию в Словасьянсе.",
    footer: "Никому не сообщай код. Если ты не регистрировался в Словасьянсе, письмо можно проигнорировать.",
  };
}
...
async function sendAuthCode(to, code, kind) {
  ...
  await smtpTransport().sendMail({
    from: { name: "Словасьянс", address: fromEmail },
    to,
